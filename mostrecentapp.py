...
# # Add to API key section (after OPENWEATHER_API_KEY)
# GEMINI_API_KEY = None
# try:
...
# # Configure Gemini (even if no key, for graceful degradation)
# if GEMINI_API_KEY:
#     try:
...
# # Add this function for Gemini interactions
...
#     try:
#         # Predefined prompt to guide Gemini's responses
#         system_prompt = f"""
...
#             st.session_state.gemini_input = "What are the best crops for clay soil with pH 6.8?"
    
#     with col2:
#         if st.button("🐛 Pest control tips"):
#             st.session_state.gemini_input = "How to control aphids in tomato plants organically?"
    
#     with col3:
#         if st.button("💧 Water management"):
#             st.session_state.gemini_input = "What's the best irrigation schedule for wheat in Rajasthan?"
    
#     # Chat input
#     if prompt := st.chat_input("Ask your farming question...", key="gemini_input"):
...
#         context = "\n".join([f"{msg['role']}: {msg['content']}" for msg in st.session_state.gemini_messages[-3:]])
